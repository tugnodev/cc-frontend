<script lang="ts">
    ...
    let currentUserAvatar =
        "https://api.dicebear.com/7.x/avataaars/svg?seed=Moi";
    ...
    <div class="flex flex-col h-full max-w-2xl mx-auto">
        ...
        >
            ...
            <div class="avatar">
                <div class="w-10 rounded-full bg-neutral text-neutral-content">
                    ...
                    {:else}
                        <span class="text-xl"
                            >{currentDiscussion.sender[0]}</span
                        >
                    {/if}
                </div>
            </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar pb-24">
            {#each currentDiscussion.messages as msg}
                <div
                    class="chat {msg.userId === '1'
                        ? 'chat-end'
                        : 'chat-start'}"
                >
                    <div class="chat-image avatar">
                        <div
                            class="w-10 rounded-full bg-neutral text-neutral-content border border-base-300"
                        >
                            {#if msg.userId === "1"}
                                <img src={currentUserAvatar} alt="Moi" />
                            {:else if currentDiscussion.avatar}
                                <img
                                    src={currentDiscussion.avatar}
                                    alt={currentDiscussion.sender}
                                />
                    ...
                    <div
                        class="chat-bubble {msg.userId === '1'
        ...
        <div
            class="fixed bottom-0 left-0 right-0 p-4 bg-base-100 flex items-center gap-2 max-w-2xl mx-auto"
        >
            <input
                type="text"
