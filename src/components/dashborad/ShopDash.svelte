	...
	$effect(() => {
		...
		salesChart = new Chart(salesCanvas.getContext('2d'), {
			...
			options: {
				...
				scales: {
					y: { ticks: { color: colorBaseContent, font: { family: 'Raleway' } } },
					x: { ticks: { color: colorBaseContent, font: { family: 'Raleway' } } }
				}
			}
		});
		...
		categoryChart = new Chart(categoryCanvas.getContext('2d'), {
			...
			data: {
				...
				datasets: [{
					data: [55, 30, 10, 5],
					backgroundColor: [colorPrimary, colorSecondary, colorAccent, colorBaseContent],
					borderWidth: 0
				}]
			},
			options: {
...
<div class="min-h-screen bg-base-100 text-base-content font-['Raleway'] p-4 md:p-8">
	...
	<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
		<div class="card bg-base-200/50 border border-base-300 shadow-sm hover:shadow-md transition-all duration-300">
			<div class="card-body flex flex-row items-center justify-between p-6">
				<div>
					<p class="text-sm font-medium text-base-content/70">Revenu Total</p>
					<h3 class="text-3xl font-bold text-primary mt-1">{kpi.revenue}</h3>
				</div>
				<div class="radial-progress text-primary text-xs font-bold" style="--value:70; --size:3rem;" role="progressbar">70%</div>
			</div>
		</div>
		...
		<div class="card bg-base-200/50 border border-base-300 shadow-sm hover:shadow-md transition-all duration-300">
			<div class="card-body flex flex-row items-center justify-between p-6">
				<div>
					<p class="text-sm font-medium text-base-content/70">Commandes</p>
					<h3 class="text-3xl font-bold text-secondary mt-1">{kpi.orders}</h3>
				</div>
				<div class="badge badge-secondary badge-lg text-secondary-content gap-2">
					▲ {kpi.growth}
				</div>
			</div>
		</div>
		...
		<div class="card bg-base-200/50 border border-base-300 shadow-sm hover:shadow-md transition-all duration-300">
			<div class="card-body flex flex-row items-center justify-between p-6">
				<div>
					<p class="text-sm font-medium text-base-content/70">Panier Moyen</p>
					<h3 class="text-3xl font-bold text-accent mt-1">{kpi.avgBasket}</h3>
				</div>
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
		<div class="card lg:col-span-2 bg-base-100 border border-base-300 shadow-xl">
			<div class="card-body">
				<div class="flex justify-between items-center mb-4">
					<h2 class="card-title text-lg">Analyse des Ventes</h2>
					<select class="select select-sm select-bordered w-32 rounded-selector bg-base-200">
				...
				<div class="relative h-[250px] w-full flex justify-center">
					...
					<div class="absolute inset-0 flex items-center justify-center pointer-events-none">
						<div class="text-center">
							<span class="block text-2xl font-bold text-base-content">100%</span>
